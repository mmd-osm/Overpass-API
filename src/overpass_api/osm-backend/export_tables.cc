               ...
               Attic<Relation_Skeleton> & m)
{
  archive(cereal::make_nvp("id",m.id),
          cereal::make_nvp("membersp",m.members()),
...
void export_map(Transaction& transaction, const File_Properties* fp) {
  ...
  for (std::vector< uint32 >::size_type i = 0; i < src_idx.get_blocks().size(); ++i)
  {
    if (src_idx.get_blocks()[i].pos != src_idx.npos)
    {
      ...
      {
        TIndex val = src_file.get(i*(src_idx.get_block_size()*src_idx.get_compression_factor()/Uint31_Index::max_size_of()) + j);
        if (!(val == TIndex{})) {
          oarchive(i*(src_idx.get_block_size()*src_idx.get_compression_factor()/Uint31_Index::max_size_of()) + j, val);
        }
      }
    }
  }

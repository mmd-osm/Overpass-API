               ...
               Attic<Relation_Skeleton> & m)
{
  archive(cereal::make_nvp("id",m.id),
          ...
          cereal::make_nvp("timestamp",Timestamp_64(Timestamp(m.timestamp)).timestamp));    // store timestamp as original 64bit value with 0x3ffff values for year field
}
               ...
               Attic<Way_Delta> & m)
{
  archive(cereal::make_nvp("id",m.id),
          ...
          cereal::make_nvp("timestamp",Timestamp_64(Timestamp(m.timestamp)).timestamp));
}
...
{
  archive(m.val(), cereal::make_nvp("timestamp",Timestamp_64(Timestamp(m.timestamp)).timestamp));
}
...
{
  auto val = m.val();
  archive(val, cereal::make_nvp("timestamp",Timestamp_64(Timestamp(m.timestamp)).timestamp));
}
...
void save(Archive & archive,
    Timestamp const & m)
{
  archive( cereal::make_nvp("timestamp",Timestamp_64(m).timestamp));
}

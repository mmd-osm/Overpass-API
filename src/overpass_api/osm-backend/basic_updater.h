...
{
  auto from_it = new_data.data.begin();
  auto to_it = new_data.data.begin();
     ...
     const File_Properties& undelete_file_properties)
{
  ...
  for (const auto & it : db.as_discrete(req))
  {
        ...
        std::make_pair(it.handle().id(), 0), comp))
    {
      ...
      if (rit == result.end())
	result.insert(std::make_pair(id, std::make_pair(it.index(), std::move(obj))));
      else if (rit->second.second.timestamp < obj.timestamp)
        rit->second = std::make_pair(it.index(), std::move(obj));
    }
  }
  ...
  for (const auto & it : undelete_db.as_discrete(req))
  {
        ...
        std::pair< typename Element_Skeleton::Id_Type, Uint31_Index >(obj, 0u), comp))
    {
      ...
      if (rit == result.end())
	result.insert(std::make_pair(obj, std::make_pair(it.index(),
     ...
     Transaction& transaction, const File_Properties& file_properties)
{
  ...
  std::set< Id_Type > req;
  auto it_pos = ids_with_position.begin();
  for (auto it = ids.begin(); it != ids.end(); ++it)
  {
  ...
  for (auto it_idx = new_attic_local_tags.begin(); it_idx != new_attic_local_tags.end(); ++it_idx)
  {
    if (it_idx->first.value != void_tag_value())
    {
      ...
      std::set< Attic< Tag_Object_Global< Id_Type > > >& void_handle
          (result[Tag_Index_Global(it_idx->first.key, std::string(void_tag_value()))]);
      for (auto it = it_idx->second.begin();
           it != it_idx->second.end(); ++it)
      {
        handle.insert(Attic< Tag_Object_Global< Id_Type > >(
...
{
  //TODO: convert the data format until existing_local_tags get the new data format
  for (auto it_idx = existing_local_tags.begin(); it_idx != existing_local_tags.end(); ++it_idx)
  {
    std::set< Id_Type >& handle(attic_local_tags[*it_idx]);
    for (auto it = it_idx->ids.begin();
         it != it_idx->ids.end(); ++it)
...
{
  for (auto it = attic_skeletons.begin();
     ...
     const std::map< Uint31_Index, std::set< Attic_Skeleton > >& full_attic)
{
  ...
  for (auto it = full_attic.begin(); it != full_attic.end(); ++it)
  {
    for (auto it2 = it->second.begin();
         it2 != it->second.end(); ++it2)
     ...
     const std::map< Uint31_Index, std::set< OSM_Element_Metadata_Skeleton< Id_Type > > >& new_meta)
{
  ...
  for (auto it = new_attic_idx_by_id_and_time.begin(); it != new_attic_idx_by_id_and_time.end(); ++it)
  {
    auto mit = meta_by_id_and_time.find(it->first);
  ...
  for (auto it = new_meta.begin(); it != new_meta.end(); ++it)
  {
    for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2)
     ...
     const std::map< Id_Type, std::set< Uint31_Index > >& existing_idx_lists)
{
  ...
  auto tit = tags_by_id_and_time.begin();
  for (auto it = new_attic_idx_by_id_and_time.begin(); it != new_attic_idx_by_id_and_time.end(); ++it)
  {
    while (tit != tags_by_id_and_time.end() && tit->first.first == it->first)
    {
      ...
      {
        for (auto iit2 = iit->second.begin(); iit2 != iit->second.end();
             ++iit2)
	  ...
	  // - or there is an even older version in this update process
          ...
          if (last_idx.val() != 0u && last_value != *tit2 && (*tit2 != void_tag_value_space()
	      || it2 != it->second.end()
		    ...
		    *tit2 != void_tag_value_space() ? std::string(*tit2) : std::string(void_tag_value()))]
	  ...
	  // and an entry at the new index that the tag wasn't std::set here before.
	      ...
	      && last_value != void_tag_value() && last_value != void_tag_value_space())
          {
            if (it2->val() != 0u)
            ...
            if (last_idx.val() != 0u)
              result[Tag_Index_Local(Uint31_Index(last_idx.val() & 0x7fffff00),
                                     tit->first.second, std::string(void_tag_value()))]
                  .insert(Attic< Id_Type >(it->first, it2->timestamp));
          }
          last_idx = *it2;
          ++it2;
	  ...
	  if (!((last_idx.val() & 0x7fffff00) == (it2->val() & 0x7fffff00)))
	  {
	    ...
	    // If the younger index is non-void then we store for it a delimiter to the past
            if (last_idx.val() != 0u && last_value != void_tag_value() && last_value != void_tag_value_space())
              result[Tag_Index_Local(Uint31_Index(last_idx.val() & 0x7fffff00),
                                     tit->first.second, std::string(void_tag_value()))]
                  .insert(Attic< Id_Type >(it->first, it2->timestamp));

	    // If the older index is non-void then we write an entry for it
            if (it2->val() != 0u && *tit2 != void_tag_value() && *tit2 != void_tag_value_space())
            {
     ...
     const std::map< Tag_Index_Local, std::set< typename Element_Skeleton::Id_Type > >& attic_local_tags)
{
  ...
  for (auto it = new_data.data.begin(); it != new_data.data.end(); ++it)
  {
	...
	if (it->meta.timestamp == timestamp_per_id[it->elem.id])
          result_ref.push_back(Attic< std::string >(std::string(void_tag_value_space()), it->meta.timestamp));
	else
	  result_ref.push_back(Attic< std::string >(std::string(void_tag_value()), it->meta.timestamp));
      }
  ...
  for (auto it = attic_local_tags.begin(); it != attic_local_tags.end(); ++it)
  {
    for (auto it2 = it->second.begin();
         it2 != it->second.end(); ++it2)
    {
...
std::unordered_map< Node_Skeleton::Id_Type, std::vector< std::pair< Uint31_Index, Attic< Node_Skeleton > > > >
    collect_nodes_by_id(
...
std::unordered_map< Way_Skeleton::Id_Type, std::vector< std::pair< Uint31_Index, Attic< Way_Skeleton::Id_Type > > > >
    collect_ways_by_id(

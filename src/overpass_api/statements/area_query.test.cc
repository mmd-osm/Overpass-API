...
void comp_sets(Resource_Manager& rman, const std::string& set_1, const std::string& set_2)
{
  ...
  auto it1(s1.nodes.begin());
  auto it2(s2.nodes.begin());
...
/*  std::vector< Aligned_Segment > segs;
...

  {
    ...
    auto* stmt1 = new Query_Statement(0, convert_c_pairs(attributes), global_settings);
    {
      const char* attributes[] = { "k", "highway", "v", "bus_stop", 0 };
      auto* stmt2 = new Has_Kv_Statement(0, convert_c_pairs(attributes), global_settings);
      stmt1->add_statement(stmt2, "");
    }
    {
      const char* attributes[] = { "ref", "3600062478", 0 };
      auto* stmt2 = new Area_Query_Statement(0, convert_c_pairs(attributes), global_settings);
      stmt1->add_statement(stmt2, "");
    }
    stmt1->execute(rman);
  }

...
int main(int argc, char *argv[])
{
  ...
  while (argpos < argc)
  {
    ...
    else if (!(strncmp(argv[argpos], "--clone-compression=", 20)))
    {
      if (std::string(argv[argpos]).substr(20) == "no")
        clone_settings.compression_method = Block_Compression::NO_COMPRESSION;
      else if (std::string(argv[argpos]).substr(20) == "gz")
        clone_settings.compression_method = Block_Compression::ZLIB_COMPRESSION;
#ifdef HAVE_LZ4
      else if (std::string(argv[argpos]).substr(20) == "lz4")
        clone_settings.compression_method = Block_Compression::LZ4_COMPRESSION;
#endif
      else
      {
#ifdef HAVE_LZ4
    ...
    else if (!(strncmp(argv[argpos], "--clone-map-compression=", 24)))
    {
      if (std::string(argv[argpos]).substr(24) == "no")
        clone_settings.map_compression_method = Block_Compression::NO_COMPRESSION;
      else if (std::string(argv[argpos]).substr(24) == "gz")
        clone_settings.map_compression_method = Block_Compression::ZLIB_COMPRESSION;
#ifdef HAVE_LZ4
      else if (std::string(argv[argpos]).substr(24) == "lz4")
        clone_settings.map_compression_method = Block_Compression::LZ4_COMPRESSION;
#endif
      else
      {
#ifdef HAVE_LZ4
  ...
  try
  {
    ...
    error_output.runtime_remark(fmt::format("Timeout is {} and maxsize is {}.", max_allowed_time, max_allowed_space));

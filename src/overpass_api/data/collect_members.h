...
std::map< Uint31_Index, std::vector< Relation_Skeleton > > relation_relation_members
    (const Statement& stmt, Resource_Manager& rman,
     ...
     const std::vector< Relation::Id_Type >& children_ids, bool invert_ids, const uint32* role_id = nullptr);
...
std::map< Uint31_Index, std::vector< Way_Skeleton > > relation_way_members
    (const Statement* stmt, Resource_Manager& rman,
     ...
     const std::vector< Way::Id_Type >& way_ids, bool invert_ids, const uint32* role_id = nullptr);
...
std::map< Uint32_Index, std::vector< Node_Skeleton > > relation_node_members
    (const Statement* stmt, Resource_Manager& rman,
     ...
     const std::vector< Node::Id_Type >& node_ids, bool invert_ids = false, const uint32* role_id = nullptr);
...
{
  filter_relations_by_ranges(relations, ranges.get_ranges());
     ...
     std::map< Index, std::vector< Attic< Skeleton > > >& attic)
{
  if (timestamp != NOW)
  {
    ...
    for (const auto & it : undeleted_db.as_discrete(idx_set))
    {
      ...
      auto cit = current.find(it.index_handle().id());
      if (cit != current.end())
      {
      ...
      auto ait = attic.find(it.index_handle().id());
      if (ait != attic.end())
      {
        for (auto it2 = ait->second.begin();
             it2 != ait->second.end(); )
        {
...
{
  for (auto it2(relations.begin());
      it2 != relations.end(); ++it2)
  {
    for (auto it3(it2->members().begin());
...
{
  std::map< typename Skeleton::Id_Type, timestamp_t > timestamp_by_id;
  ...
  for (auto it = rels_begin; it != rels_end; ++it)
  {
    if ((it->first.val() & 0x80000000) && ((it->first.val() & 0x3) == 0))
    {
      ...
      for (auto it2 = it->second.begin();
          it2 != it->second.end(); ++it2)
      {
	for (auto it3 = it2->node_idxs().begin();
...
{
  for (auto it1 = arg.begin(); it1 != arg.end(); ++it1)
  {

...
const Ranges < Uint32_Index >& Bbox_Filter::get_ranges_32() const
{
  ...
  {
    ranges_32 = Ranges< Uint32_Index >(::get_ranges_32(bbox.south, bbox.north, bbox.west, bbox.east));
  }
  return ranges_32;
}
...
const Ranges < Uint31_Index >& Bbox_Filter::get_ranges_31() const
{
  if (ranges_31.empty())
  {
    ranges_31 = Ranges< Uint31_Index >(calc_parents(get_ranges_32().get_ranges()));
  }
  return ranges_31;
}
...
void Bbox_Filter::filter(Set& into) const
{
  ...
  const auto & ranges = get_ranges_31();
